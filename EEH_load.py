...
state = ['After_Soc','Before_Soc']

for line in range(2):
    wb = Workbook()
    wb.save(filename='{}_EEH.xlsx'.format(state[line]))
    for n in range(10):
        
        if (2*n<9): x = pd.read_csv('{}/1/000{}_X'.format(state[line],2*n), skiprows=10, sep=')', names = ['time','X'],index_col = False)
        if (2*n<9): y = pd.read_csv('{}/1/000{}_Y'.format(state[line],2*n), skiprows=10, sep=')', names = ['time','Y'],index_col = False)
        if (2*n<9): z = pd.read_csv('{}/1/000{}_PZ'.format(state[line],2*n+1), skiprows=10, sep=')',names = ['time','Z'],index_col = False)
        
        if (2*n>9): x = pd.read_csv('{}/2/000{}_X'.format(state[line],2*(n-5)), skiprows=10, sep=')', names = ['time','X'],index_col = False)
        if (2*n>9): y = pd.read_csv('{}/2/000{}_Y'.format(state[line],2*(n-5)), skiprows=10, sep=')', names = ['time','Y'],index_col = False)
        if (2*n>9): z = pd.read_csv('{}/2/000{}_PZ'.format(state[line],2*(n-5)+1), skiprows=10, sep=')',names = ['time','Z'],index_col = False)
        ...
        book = load_workbook('{}_EEH.xlsx'.format(state[line]))
        writer = pd.ExcelWriter('{}_EEH.xlsx'.format(state[line]), engine='openpyxl') 
        writer.book = book
